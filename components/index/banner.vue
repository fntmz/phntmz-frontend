...
const state = reactive({ current_word: 0 });

let words = ["interstellar", "divinity", "surrealism"];

function nextWord() {
    if (state.current_word === words.length - 1) {
...
<template>
    ...
    >
        ...
        >
            <span
                class="interstellar-letters"
                v-for="letter in words[state.current_word]"
            >
                {{ letter }}
            </span>
        </h1>
